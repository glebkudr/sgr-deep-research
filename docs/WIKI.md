...
## GraphView: кластеризация по файлам (path)

- Как включить:
  - Откройте страницу /graphview.
  - Заполните поля Collection/Limit и при необходимости Rels.
  - При желании укажите Seeds (для client‑режима обязательно; для server — обязательно).
  - Включите чекбокс "Cluster by file (path)" и нажмите "Load". Опция применяется при перезагрузке.
- Что происходит:
  - Для каждого уникального file path создаётся явный узел с label="File" (id: file::<full_path>, title: полный путь).
  - Для каждого узла с path добавляется связь IN_FILE к своему File‑узлу.
  - Настраиваются силы для d3-force: IN_FILE связи имеют повышенную силу и меньшую дистанцию, чтобы «собирать» кластер.
- Ограничения:
  - Узлы без path не кластеризуются (не добавляется File‑узел и IN_FILE связь).
  - Число элементов растёт за счёт File‑нод и IN_FILE связей — это влияет на производительность и читаемость.
  - Параметр включается на загрузке данных; после смены состояния чекбокса нажмите Load для применения.
- Параметры сил (по умолчанию в UI):
